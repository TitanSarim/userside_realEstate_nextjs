...
import {allAgent, getProjects} from '../../../redux-toolkit/actions/action'
import TopTitleP from "../stickyTabOrClassic/TopTitlep";
import ImageSectionP from "./ImageP";
import SingleProjectSection from "./SingleProjectSection";


const BodyContentP = ({project}) => {
  ...
  const agent = agents.find((agent) => agent.userid === project?.project?.[0]?.userid);

  const relatedProjects = projects.filter((prop) => prop.project_type === project?.project?.[0]?.project_type && prop.id !== project?.project[0]?.id);
    ...
    <>
      ...
      <section className="single-property mt-0 pt-0">
          ...
          <Row className=" ratio_55">
            <Col xl="9" lg="8">
              <div className="description-section tab-description">
                <ImageSectionP project={project}/>
                {/* <GalleryBox project={project}/> */}
                <SingleProjectSection project={project}/>
                {/* <ReviewsDeskBox /> */}
              </div>
            </Col>
            <Sidebar>
        ...
        <div className="related-abjust">
          <RelatedProperty related={relatedProjects}/>
        </div>
      </section>
    </>
  );
